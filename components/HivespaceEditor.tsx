  ...
  addNodeView() {
    return ({ node }) => {
      ...
      dot.className = cn(
        ...
        priority === "urgent" ? "bg-red-500" : priority === "high" ? "bg-amber-500" : "bg-zinc-500"
      );
      ...
      avatar.innerHTML = `<span class="text-[10px] text-zinc-300 font-bold">${assignee.split(" ").map((n: string) => n[0]).join("")}</span>`;
...
export function HivespaceEditor({ initialContent, onUpdate }: HivespaceEditorProps) {
  ...
  const editor = useEditor({
    ...
    extensions: [
      StarterKit.configure({
        codeBlock: {
          HTMLAttributes: {
            class: "bg-zinc-800 rounded-lg p-4 font-mono text-xs text-zinc-300 border border-zinc-700/50 shadow-inner my-4",
          },
        },
        heading: {
          HTMLAttributes: {
    ...
    editorProps: {
      attributes: {
        class: "prose prose-invert max-w-none focus:outline-none text-zinc-300 leading-[1.8] text-[15px]",
      },
  ...
  return (
    <div className="relative w-full h-full">
      ...
      {isSlashMenuOpen && (
        <div 
          className="fixed z-[100] bg-zinc-900 border border-zinc-800 rounded-lg shadow-2xl w-64 p-1 animate-in fade-in slide-in-from-top-2 duration-200"
          style={{ top: menuPos.top, left: menuPos.left }}
        >
          <div className="px-3 py-2 flex items-center gap-2 border-b border-zinc-800/50 mb-1">
            <Search className="h-3 w-3 text-zinc-500" />
            <span className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest">Editor Commands</span>
          </div>
          <div className="max-h-64 overflow-y-auto scrollbar-none pb-1">
            <SlashItem onClick={() => insertBlock("h1")} icon={Heading1} label="Heading 1" shortcut="⌘ 1" />
            <SlashItem onClick={() => insertBlock("h2")} icon={Heading2} label="Heading 2" shortcut="⌘ 2" />
            <SlashItem onClick={() => insertBlock("bulletList")} icon={List} label="Bullet List" shortcut="-" />
            <SlashItem onClick={() => insertBlock("codeBlock")} icon={CodeIcon} label="Code Block" shortcut="```" />
            <SlashItem onClick={() => insertBlock("blockquote")} icon={Quote} label="Quote" shortcut=">" />
            
            <div className="px-3 py-1.5 mt-2 flex items-center justify-between">
               <span className="text-[10px] font-bold text-zinc-600 uppercase tracking-widest">Hivespace Native</span>
            </div>
            <SlashItem 
            ...
            />
             <SlashItem 
              ...
              className="text-violet-400 font-medium hover:bg-violet-500/10"
            />
          </div>
        </div>
      )}
      ...
      <style jsx global>{`
        ...
        .ProseMirror h1 { font-size: 1.875rem; line-height: 2.25rem; font-weight: 700; color: white; margin-top: 2rem; margin-bottom: 1rem; }
        .ProseMirror h2 { font-size: 1.25rem; line-height: 1.75rem; font-weight: 700; color: white; margin-top: 1.5rem; margin-bottom: 0.75rem; border-bottom: 1px solid #27272a; padding-bottom: 0.5rem; }
        .ProseMirror h3 { font-size: 1.125rem; line-height: 1.75rem; font-weight: 700; color: white; margin-top: 1.25rem; margin-bottom: 0.5rem; }
        ...
        .ProseMirror blockquote { border-left: 3px solid #7c5cfc; padding-left: 1rem; color: #a1a1aa; font-style: italic; margin: 1.5rem 0; }
        .ProseMirror code { font-family: 'Geist Mono', monospace; background: #27272a; padding: 0.2rem 0.4rem; rounded: 0.25rem; font-size: 0.85em; color: #e4e4e7; }
...
function MenuButton({ active, onClick, icon: Icon }: { active: boolean; onClick: () => void; icon: any }) {
  return (
    <button 
      onClick={onClick}
      className={cn(
        "h-7 w-8 flex items-center justify-center rounded transition-all",
        active ? "bg-violet-600 text-white shadow shadow-black/30 scale-105" : "text-zinc-400 hover:text-zinc-200 hover:bg-zinc-800"
      )}
    >
...
function SlashItem({ 
  ...
  return (
    <div 
      onClick={onClick}
      className={cn(
        "flex flex-col gap-0.5 px-2 py-2 cursor-pointer rounded-md transition-colors hover:bg-zinc-800 group relative",
        className
      )}
    >
      <div className="flex items-center gap-2.5">
        <div className="h-6 w-6 rounded border border-zinc-800 bg-zinc-800/50 flex items-center justify-center shrink-0 group-hover:border-zinc-700 transition-colors">
          <Icon className="h-3.5 w-3.5 text-zinc-500 group-hover:text-white" />
        </div>
        <div className="flex flex-col">
          <span className="text-sm font-medium">{label}</span>
          {info && <span className="text-[10px] text-zinc-600 leading-none">{info}</span>}
        </div>
        {shortcut && <span className="text-[10px] text-zinc-700 font-mono ml-auto group-hover:text-zinc-500">{shortcut}</span>}
      </div>
    </div>
  );
}
